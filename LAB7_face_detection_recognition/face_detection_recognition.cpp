...
int main(int argc, char** argv)
{
	CommandLineParser parser(argc, argv,
		...
		"{image1 i1         |  jensen.jpg    | Path to the input image1. Omit for detecting through VideoCapture}"
		"{image2 i2         |  mutiple.jpg       | Path to the input image2. When image1 and image2 parameters given then the program try to find a face on both images and runs face recognition algorithm}"
		...
		"{save s            | true      | Set true to save results. This flag is invalid when using camera}"
	);
	if (parser.has("help"))
	{
		parser.printMessage();
